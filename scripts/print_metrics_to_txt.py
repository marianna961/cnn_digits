...
def preprocess_image(image, img_size=(32, 32), method=None):
    if method == 'blur_binary':
        ...
        raise ValueError("blur_binary, adaptive_binary, Nothing")
...
def validate_roi(x, y, w, h, max_width, max_height):
    """Проверка Roi допустимых границ."""
...
def process_video(video_path, camera_params, metrics_data, onnx_session, output_dir, method=None):
    ...
    # Преобразование секунд в номера кадров
    ...
    # Хранение истинных и предсказанных меток
    ...
    print("Метрики сохранены в", output_path)

    return metrics

def main():
    onnx_model_path = "models/40e_resnet18_adaptive_binary.onnx"
    video_dir = "sip_12022025/sip_12022025"
    metrics_dir = "metrics_params"
    camera_params_path = "configs/camera_params.json"
    output_dir = "metrics_40e_resnet18_adaptive_binary"
    ...
    for metrics_file in os.listdir(metrics_dir):
        if metrics_file.endswith('.json'):
            ...
            print("Текущие видео:", video_name)
    ...
    #  Вычисление и сохранение средних
    if all_metrics:
        ...
        print("mean saved in", avg_output_path)
