...
def preprocess_image(image, img_size=(32, 32), method=None):
    if method == 'binary':
        ...
        raise ValueError("binary or adaptive_binary")
...
def process_video(video_path, camera_params, metrics_data, onnx_session, output_dir, method=None):
    ...
    output_path = os.path.join(output_dir, f"{video_name}_metrics.json")
...
def main():
    ...
    output_dir = "metrics_40e_resnet18_classic"
    method = "Nothing"
    ...
    for metrics_file in os.listdir(metrics_dir):
        if metrics_file.endswith('.json'):
            ...
            if os.path.exists(video_path):
                ...
                process_video(video_path, camera_params, metrics_data, session, output_dir, method=method)
            else:
